...
def group_a():
    ...
    # ---- ★★ 2026-09-26 删掉了原来这条：「商品总数是 100 件」 ----
    #
    #   它原来是 `check("★ 商品总数是 100 件（用户拍板「全部商品都配 SKU」）",
    #                   total == 100, ...)`。
    #
    #   删掉它，因为它把【判据】写成了【快照】：
    #     它想守的是「里程碑 15 的 SKU 化覆盖了全部商品」，
    #     而那个判据已经由上面那条守住了 —— 而且是用【不依赖总数】的方式：
    #       · 上面那条：product → product_sku 方向（每件商品都有 SKU）
    #       · :745 那条：product_sku → product 方向（没有孤儿 SKU 行）
    #     两个方向合起来就是一个充要条件，和「库里正好有 100 件商品」无关。
    #
    #   ★ 实际发生的事：用户手工录入商品（这是正常使用，README 里写了
    #     商城有用户自己录的商品），商品数变成 101 —— 这条断言就红了，
    #     而它想守的那件事好端端的。
    #     这正是本项目反复踩的那类坑的反面：以前是「一份谁都不量的清单
    #     一定会过期」（里程碑 17 的 VALID_ORDER_STATUS）；这次是
    #     「把一个数字当成不变量，它一定会过期」。
    #   ★ 判据：断言里出现「正好 N 件」这种绝对数字时，先问一句
    #     「这个 N 是【不变量】还是【我写这条断言那天的样子】」。
    #     后者不该写成断言 —— 它只会在有人正常使用系统时变红，
    #     然后被下一个人调大成 101、102…… 直到它彻底失去意义。
