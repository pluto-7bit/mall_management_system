...
const routes = [
  ...
  },
  {
    // 售后管理。★ 里程碑 17 加的。
    //
    // ⚠️ 路径用【连字符】`/after-sale`，不是 `/afterSale`。
    //   理由和 `/order` 用单数一样：URL 是给人看的、也是给人念的，
    //   而「大小写」在念出来的时候是不存在的 ——
    //   客服在电话里说「你打开 /aftersale」，对面输成什么全看运气。
    //   全小写加连字符是唯一不需要解释的写法。
    //
    //   和上面几条一样的硬要求：路径字符串必须和 App.vue 里
    //   <el-menu-item index="/after-sale"> 【逐字相同】。
    //   差一个字母的症状是「点进去 404，而且菜单永远不高亮」。
    //
    // ★ 它和 `/order` 的关系：售后单属于订单，所以菜单里紧挨着放。
    //   但它们是【两条不同的链路】，不要试图把售后做进订单页 ——
    //   售后有自己的状态机（六个状态）、自己的列表、
    //   而且管理员的动作完全不同（订单页是「发货」，这里没有发货）。
    path: '/after-sale',
    name: 'AfterSaleList',
