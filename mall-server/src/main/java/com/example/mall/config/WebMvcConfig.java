...
public class WebMvcConfig implements WebMvcConfigurer {
    ...
    public void addInterceptors(InterceptorRegistry registry) {
        ...
        registry.addInterceptor(memberAuthInterceptor)
                ...
                //
                // ★★ 里程碑 15 阶段 3 又验证了一次这条规矩，而这次是【反面】的：
                //     SKU 读取接口挂在了 /api/shop/skus/** —— 一棵【新树】，
                //     不在上面那两棵里面。于是它一个字都没沾到这份清单，
                //     拦截器把它当成了要登录的接口（测试当场拿到 401）。
                //     ⚠️ 而这正是上面那句「清单开始说假话」的另一种表现：
                //        清单没说假话，它【什么都没说】——
                //        权限照旧生效，只是生效成了错的那一种。
                //     ★ 所以规矩要说得更一般些：
                //        **新增一棵匿名可读的路径树时，必须回来补一行。**
                ...
                .excludePathPatterns(
                        ...
                        "/api/shop/categories",
                        // SKU 读取：GET /api/shop/skus/{skuId} ★ 里程碑 15 阶段 3 新增
                        //
                        // ★ 它该不该匿名，判据不是「现在有没有游客调它」（暂时没有），
                        //   而是【它泄露的东西是不是已经匿名可得了】。
                        //   这个接口返回的每一个字段 —— 商品名、封面、分类、
                        //   规格、单价、库存 —— 商品详情接口
                        //   （GET /api/shop/products/{id}，同级匿名）原样都返回。
                        //   所以放开它带来的【新增】信息泄露是零。
                        //
                        // ⚠️ 反过来才是要注意的：如果哪天有人往这个响应里加了
                        //    详情接口【没有】的字段（比如成本价、真实的进货量），
                        //    这里就不再成立了。加字段时回来读一遍这段话。
                        //
                        // ⚠️ 只放开 GET。这批 SKU 接口全都是只读的，
                        //    这个路径树下不该出现任何写操作（见下面那条红线）。
                        "/api/shop/skus/**");
